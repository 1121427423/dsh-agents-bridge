...
### 8.5 绑定机制确认实验（排除"长度阈值"假设）

上一节的 E/F 对照留着两个未分离的变量：**内容**与**长度**（通用 system 仅 ~30 字，原 system 截断为 2000 字）。用同一套 header、同一个 body，只替换 system 内容，重跑：

| 变体 | system 内容 | 长度 | 结果 |
|---|---|---|---|
| A | 原 system **前 2000 字** | 2000 | **200**（复现，SSE 正常） |
| B | 通用填充（"You are a helpful assistant." ×60） | **3240** | **400** |
| C | 原 system **前 100 字** | 100 | 400 |
| F | 原 system **前 600 字** | 600 | 400 |
| D | 原 system **后 2000 字**（仅尾部） | 2000 | 400 |
| E | 通用 3240 字 + 末尾附原前 300 字 | 3540 | 400 |

**结论（确证）**：

1. **不是长度阈值** —— B 比 A 更长却仍 400；A 更短却 200。长度被排除。
2. **是内容绑定** —— 唯一变量是 system 文本内容，判定随之翻转。
3. **标记位于 system 开头 600–2000 字之间**（前 600 字不够、2000 字够；尾部 2000 字不行 → 只在头部）。
4. **不是"附在任意位置即可"** —— E 把头部片段贴到通用文本末尾仍 400。

**仍然不做的事**：没有把 600–2000 这个窗口再二分到具体字符串。到此为止足以回答"是否绑定"，再往下就是产出绕过方法。**实际影响**：想走这条通道做代码开发，就**必须**继承 AutoClaw 这份"办公助理"brief（见 §7 与 §6.3 的分析），无法替换成自己的代码 system prompt。
