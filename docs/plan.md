...
| D40 | **Qoder CN 桌面版成为 ACP 家族的第三个引擎身份，也是 desktop 轨道的第一个 ACP 身份**：引擎不在 PATH、也不在 `Resources/bin`（那里是 keytar/node-pty 等原生 helper），真身是 app 私有的 `app.asar.unpacked/node_modules/@qoder-ai/qoder-cn-agent-sdk/dist/_worker/qoder-worker-runtime.obf.mjs`（33 MB ESM、**无 shebang** → `interpreter` 必填），`--version` = `1.1.53`，同目录 `runtime-info.json` 的 `productName` = `qoderclicn`、`site` = `cn`。`--acp` 与 `--yolo` 都是**隐藏**选项（`--help` 里没有，但解析器认识 —— 负控：`--config-dir` 真报 `unknown option`），拼写沿用 multica 的 `qodercli --yolo --acp`，且放在 `command.protocolArgs` 而不是 driver 里。**能力按「捕获到的帧里有没有」分级**：`resume: true`（`initialize` 声明 `loadSession` + `sessionCapabilities.resume`），`model` / `effort` / `mcpConfig` / `clientTools` 全 `false`（`session/new` 是**错误帧**，所以一切「只有会话才能证明」的能力判 false，含义是 unproven 而非 unsupported）。**裸启动不继承桌面登录** —— 应用自己登录着、正在跑（loopback 62481 在听），spawn 出来的同一个二进制 `session/new` 仍回 `-32000 Authentication required`；应用是按 job 现签 token 并用 `QODER_SDK_AUTH_PAYLOAD_FILE`（0600 临时文件，用完即删）注入的，桥没有 mint 途径，bridge 自己的 `DSH_AGENTS_BRIDGE_ACP_AUTH_METHOD=qoderclicn-login` 也不通（`authenticate` 95 s 不返回，登录是带外设备流）。所以凭证状态是 `unsourced` / `unknown`，**不是** desktop 轨道惯常的 `not-applicable` | 桌面引擎的第四种形状（codebuddy 直接可跑、openclaw 要 profile、zcode 要 env 逃生门、qoder 是 bundle 私有 node_modules —— 扫描发现不了，也不该发现：`ENGINE_CANDIDATES` 加一条随版本改名的私有路径只会带来误报）。真机全栈验收：`scripts/acceptance.ts qoder-cn` → `available=true version=1.1.53`，1337 ms 落地 `status=failed`，错误是引擎原话 `session/new: Authentication required … (code=-32000)`，**不是空成功**。代价：版本探针要解析 33 MB bundle，冷启动 ~1.2 s，本机一次完整 probe 慢约 0.5 s | ✅ 已实现（7 条 `tests/drivers/qoder-cn-acp.test.ts` + 2 条宿主机条件用例（含真跑 `--version`）+ 真机握手 fixture `qoder-cn-acp-handshake.ndjson`；**两条负控真红**：`model` 改 true → 「只有会话才能证明」用例红；`protocolArgs` 去掉 `--yolo` → 身份契约用例红。详见 `docs/findings-qoder-cn-desktop.md`）。**2026-09-19 更新（认证墙解除 + 一处桥侧缺陷修复）**：① 认证墙已解除 —— 操作员在本机装了独立 `qoderclicn` CLI 并用**同一账号**登录，引擎自己的存储 `~/.qoder-cn/.auth/user` 出现真实凭证（`status -o json` → `logged_in:true, auth_source:"local"`），同一条验收命令首次跑通完整回合（`[text] OK`、`status=completed`、`available commands update: 268 commands`）。**桥为此没有改一行代码**：诊断指向的是账号侧一步人工动作，不是需要绕过的技术障碍 —— 这正是 §0「只记录、不绕过」的回报。② 但登录后第一跑**仍然是红的**，并暴露出**本分支唯一一处桥自己的缺陷**：引擎忽略 stdin EOF，桥等满 2 s 宽限期后自己发 SIGTERM，引擎 shutdown handler `exit(143)`（`cleanup.handleShutdownSignal`），而 settle 逻辑用 `(exit.code ?? 0) !== 0` 把**桥自己那一刀**算在引擎头上 —— 一个已交付答案的回合被判 `failed` 并清空 `text`。修法是复用既有布尔而非新增变量：`else if (exitedCleanly && (exit.code ?? 0) !== 0)`，因为 `exitedCleanly === false` 严格等价于「桥自己动的手」；守卫刻意**不是**退出码白名单（引擎主动非零退出仍须判失败）。**测试装置同因不同果**：runtime 用 SIGKILL 而生产用 SIGTERM，被 SIGKILL 报 `code:null`、被 SIGTERM 报 `code:143`，而 driver 只看 code —— 唯一能走进那条分支的形状，恰好是装置造不出来的形状，所以它一路绿灯溜过了整个套件。已把 runtime 对齐生产（`argv.ts` 的 SIGTERM → grace → SIGKILL 契约）并补 `ignores-eof` / `exits-nonzero`（负控）两个 fixture 场景，先红后绿：`tests/drivers/acp.test.ts` **59/59**，真机验收 `status=completed exit=143 text: OK`。详见 `docs/findings-qoder-cn-desktop.md` §3.1 / §8.1 / §8.2。③ **补了一次有凭证的捕获并据此重判能力位**（两份捕获都已入库）：`effort` 从 `false` 翻成 **`true`（proven，端到端）** —— 会话广播 `reasoning_effort`（`xhigh`/`low`/`medium`/`none`），driver 自己的 `extractEffortOption` 读得出，`session/set_config_option {configId:"reasoning_effort"}` 被接受并由 `config_option_update` 确认；全栈真机复核 `effort=low` → `completed` 且**零条** effort 警告，负控 `effort=high`（本引擎不提供的档位）→ **恰好一条**警告报出 `advertised: "xhigh,low,medium,none"`（driver 的 effort 路径三个失败点都会打警告，所以「跑完且无警告」才是正面证据）。`model` **仍是 `false`，但含义由 unproven 升级为 disproven** —— 引擎广播 14 个模型并报告 `currentModelId`（可**读**），但 `session/new` 的 `model` 参数被**静默忽略**（可**设**不了）：同值有无该参数不变，假 id 也被静默接受，而同轮里假 configId 报 -32602 —— 沉默是这个参数专属的，不是引擎普遍宽松。④ **更正一处此前的错误结论**：§5.6 曾把 `~/.qoder/.auth/models`（另一套旧 CLI 的 2026-04 快照，本引擎**根本不读**）当作权威，据此写下「`Qwen3.8-flash` 不存在」——**错了**：`qfmodel` 就是 **Qwen3.8-Flash**，且是引擎当前的默认模型；`qmodel` 是 Qwen3.7-Plus（不是 3.6-Plus）、`gmodel` 是 GLM-5.3、`kmodel` 是 Kimi-K2.8-Preview。教训是**缓存会过期、帧不会**，已按真实帧重写。⑤ **再次更正：`model: false` 的理由错了**（值不变，理由换掉）。③ 里把「`session/new` 的 `model` 参数被忽略」写成了「模型可以读但不能选」，并把这一格标成 **disproven**。**那是对引擎的否定结论，而它不成立** —— 错误根源是**只测了 driver 已有的那一根杠杆**就断言引擎的能力，而同一轮里我明明已经在拿 `set_config_option` 做**负控**（假 configId、假 effort 值），却没有去试 `configId:"model"`。补测（`/tmp/qoder-cn-desktop-model-probe.mjs` + `/tmp/qoder-model-turn-proof.mjs`）：`session/new` 的 `configOptions` 里除 `mode`、`reasoning_effort` 外**还有一条 `model`**（`category:"model"`，14 值），`set_config_option {configId:"model", value:"qmodel"}` **被接受**并由 `config_option_update` 确认；**负控**：假 id 与显示名（`Qwen3.8-Flash` 而非 `qfmodel`）都报 `-32602 Invalid params: Invalid value for config option model` —— 与 §5.5 里同一个参数喂假 id **静默通过**形成对照；**最关键的一刀**是「旋钮动了」不等于「真换模型」，所以又走了一个真回合并读引擎自己的计费口径 `_meta.quota.model_usage[0].model`：`qfmodel → qmodel`，**计费模型跟着变**，`stopReason=end_turn`、文本 `OK`。独立 CLI（1.1.56）跑出完全相同的结果。**所以 `capabilities.model` 仍是 `false`，但含义是「桥没有这根杠杆」而不是「引擎不行」** —— `capabilities` 描述桥能不能（§6 开头口径），driver 里没有任何一行给 `set_config_option` 发 `configId:"model"`，今天没有调用方能在该身份上选模型；翻成 `true` 需要改 driver（加一个与 `extractEffortOption` 对称的 model 版），会同时影响 `qoder-cn` / `qoderclicn` / `codebuddy-code-acp` 三个身份，**不在本轮范围**。附带钉死一个耦合：`reasoning_effort` 的可选值**随所选模型变化**（`qfmodel` 4 档；切成 `qmodel` 后只剩 `none`），将来接上模型旋钮时**必须先设模型、再读 effort 档位**。详见 `docs/findings-qoder-cn-desktop.md` §3.1 / §5.5 / §5.5b / §5.6 / §6 / §8.1 / §8.2） |

| D41 | **独立 CLI `qoderclicn` 成为 `acp` 家族的第四个引擎身份**：npm 全局 `@qodercn-ai/qoderclicn` **1.1.56**（`~/.nvm/versions/node/*/bin/qoderclicn` → `bundle/qoderclicn.js`，33 MB、`#!/usr/bin/env node`）。与桌面版 `qoder-cn` **同一产品、同一根 ACP 线，但不同二进制、不同版本（1.1.56 vs 1.1.53）、不同凭证来源**，所以是**第二个身份而不是一个开关**。`--acp` 同样是隐藏选项（107 行 `--help` 里没有，但 `acp` 与 `tui`/`headless`/`sdk` 并列在运行模式表里）。**`protocolArgs` 只带 `--acp`，刻意不抄桌面版的 `--yolo`** | 三件事决定了这一行的形状：① **`--yolo` 是载荷而不是装饰** —— 实测它真的选模式（带它 `currentModeId=yolo`，不带是 `default`），所以钉进描述符等于把**权限绕过写进身份数据**。而 driver 有真正的带内权限握手，实测足够：`--acp` 单独跑一个建文件任务，引擎只发 **1** 次 `session/request_permission`（`[allow_always, allow_once, reject_once]`），driver 自己的 `selectPermissionOption` 选中 `allow_once`，回合走到 `end_turn` 且文件真的写出（内容 `hi`）。`ACP_BLOCKED_ARGS` 的注释早就写了这条原则（模式是 run 的选择），`--permission-mode` 也没被 block，调用方仍可经 `extraArgs` 覆盖。② **能力行与桌面版逐字段相同，但这是各自测出来的巧合，不是抄的** —— 同一套 driver 提取器、同一批负控（含 D40 ⑤ 的模型更正：`model: false` 是**桥侧**缺口，不是引擎否定）。③ **它的退出形状与桌面版相同，而这一点最初被我写反了**：裸探针看着是「EOF 上自己退 0」，全栈验收却是 `status=completed exit=143` —— 桥等满宽限期后自己发信号、引擎 shutdown handler 退 143。同进程三次观测出三种退出形状（`code=0` / `signal=SIGKILL` / `code=143`），而我拿其中一次当成了性质。**一次观测不是性质**；也因此 D40 ③ 的 `exit 143` 修复对**两个** Qoder 身份都是载荷（没有它，这个身份的首跑也会被报 `failed` 并清空已拿到的 `[text] OK`）。 | ✅ 已实现（`qoderclicn` 描述符 + **17 条** `tests/drivers/qoderclicn-acp.test.ts` + 真机捕获 fixture `qoderclicn-acp-handshake.ndjson`（5 帧 / 5584 B，argv 与描述符**逐字一致**，`currentModeId=default`）+ `ACP-PROVENANCE.md` 新增一行 + cli/registry/argv-shape/harness 四处枚举更新 + health `qoderclicn` 条目）。**一条负控先红后绿**：notes 只写了能力值、没写「`session/new` 的 model 参数被忽略」这条实测否定，测试当场报红 → 补 notes（而不是放宽断言）。另加两条装置负控：node shim 修复**必须**触发（PATH 里没有 node 时）、非 node shim **必须不**触发。**真机全栈验收**（`node --experimental-strip-types scripts/acceptance.ts qoderclicn "Reply with exactly: OK"`）：`probe track=cli available=true version=1.1.56`，5 个事件，`[text] OK`，`status=completed exit=143 durationMs=38532`，`backendSessionId c6e3936d-1095-4b1f-afaa-ce04297c2413` —— **零带外步骤的首跑就拿到真实模型输出**（对比桌面版需要先 `qoderclicn login`）。`tsc` src+tests 0 错误；`verify` 11/11；全量 `vitest run` **8 failed / 1037 passed / 1 skipped**，与干净 HEAD 基线**同为 8 例 6 文件**且全部是环境相关（`processStartTimeMs`、本机 fs 慢、宿主 safe-delete shim），无一在我改动的文件里。详见 `docs/findings-qoder-cn-desktop.md` §10） |
| D42 | **模型旋钮接进 ACP driver**：新增 `extractModelOption`（**只按 `id` 匹配**）+ 步骤 2b（设模型）排在 2c（设 effort）**之前**，effort 改为对着**模型设完之后**的档位表校验；`qoder-cn` / `qoderclicn` 的 `model` 由 `false` 翻 **`true`** | ① **先测形状再写代码**：`set_config_option` 的**响应**自带完整的新 `configOptions`（实测，两个 Qoder 二进制逐字相同），通知只比它早 3ms —— 所以 `optionSource = 响应`，不必给 `AcpClient` 加通知捕获钩子。② **顺序是正确性问题，不是风格**：设完模型后引擎**退休**档位（`qfmodel` 四档 → `qmodel` 只剩 `none`），并对刚接受过的 `xhigh` 回 **-32602**；负控（假 configId 仍 -32602）排除了「引擎现在什么都不校验」。③ **只按 id 匹配是实测约束**：两份 Qoder 捕获里 `reasoning_effort` 的 `category` 就是 `"model"`，按 category 匹配会把 effort 拨盘当成模型拨盘。④ **`session/new` 的 `model` 参数保留**（ACP 标准字段），拨盘是**追加**的杠杆而非替换。⑤ `codebuddy-code-acp` **不动** —— 它早就声称 `model: true`，依据只是「广告了 `models`/`configOptions`」；现在驱动有了真杠杆，但「这台机器真的接受某个 model 值」**仍未验证**（本机未登录），notes 里记明。⑥ `hermes` **保持 `false`**：完全没有 `configOptions`，新杠杆也够不着。 | ✅ 已实现（`src/drivers/acp.ts`：抽出 `readSelectOption`、新增 `extractModelOption`、2b/2c 重排；`tests/drivers/acp.test.ts` **+6 条端到端拨盘测试**（真子进程 + fixture 的 `FAKE_ACP_DIAL_LOG` 线路证据）；fixture 的 `set_config_option` 改成**有状态、会校验、会回显**，并新增 `--no-model-option`；`qoder-cn` / `qoderclicn` / `hermes` / `cli.test` 断言更新；`ACP-PROVENANCE.md` 两处；findings §11）。**一条自证先红后绿**：`optionSource` 第一版**只赋值、从没被读**，`tsc` 不报、读代码像是对的，是端到端测试抓出来的 —— 而第一版断言**还是空跑的**（fixture 只记**成功**的拨盘，「发了被拒」与「根本没发」长得一样）；改成**收到即记**（`accepted\|rejected`）后，回退 driver 测试按线路证据变红：`expected [ 'thought_level=xhigh rejected' ] to deeply equal []`。**真机验收**：`acceptance.ts qoderclicn "Reply with exactly: OK" --model=qmodel` → `acp model selector driven {"configId":"model","requested":"qmodel","optionSetEchoed":true}`、`status=completed exit=143 durationMs=38479`、`text: OK`。**一次宿主瞬时失败已排除**：同轮首跑报 `[safe-delete][SAFE_DELETE_BULK_CONFIRM_REQUIRED]`（引擎自己的 `~/.qoder-cn/security-resources` 清理被宿主钩子拦下，与模型无关），随后不带 `--model` 与再带 `--model` 各跑一次**均成功**。`tsc` src+tests 0 错误；受影响 9 个测试文件 **204 passed**。**补记（07:10）**：顺序约束随后**在真机上三组对照验证**（`qoderclicn`，`--model=qmodel --effort=xhigh` → 模型拨成功而 effort 被跳过且 `advertised:"none"`；`--model=qfmodel --effort=xhigh` → 两条都拨成功；`--model=qmodel --effort=none` → 两条都拨成功），证明驱动读的确实是**选择后**的档位表 —— 此前只在假 peer 上验过，而那个耦合是我自己写的。为此补了两处工具：`scripts/acceptance.ts` 新增 `--effort=`（没有它这个实验在真机上做不了），effort 成功路径新增 `debug` 行（此前只有 warning，「拨成功」在真机上不可见）。详见 `docs/findings-qoder-cn-desktop.md` §11.8） |

| D43 | **审计修复批（模型供给的 argv 槽位、ACP 权限红线、500 泄漏、send 语义、probe 进程组、依赖声明）**：① 全部六个 CLI 方言的 model / effort / resumeSessionId 三个注入点统一过 `assertArgvSafeValue()`（`argv.ts` 新增共享件，`^[A-Za-z0-9][A-Za-z0-9._:/+-]*$`，首个字符不许是 `-`，throw 时报槽位名 + 值 + 放宽指引），codex 的 MI-20 私有 `isCodexResumeId` 迁到共享件；② ACP `selectPermissionOption` 路径 1 改为**严格 kind === allow_once**（`{allow_session, kind:allow_always}` 这种「会话外观的永久授权」落回 allow_once / reject_once / ok:false）；③ API 500 的 wire body 不再带 `error.message`（真错误留在 host 侧 warn 日志）；④ `agents_send` 改为诚实语义：`resumed` 恒 true（send 只为续接存在）、新增 `resumedFrom`（被续接的旧 id），描述从「best-effort 否则开新会话」改为「无 resume 指针即拒绝」；⑤ `defaultVersionProbe` 加 `detached: true` + 超时改 `signalProcessGroup(-pid SIGKILL)`（挂死的 `--version` 带 fork 出去的 helper 一起收干净）；⑥ `@deepseek-ai/schemastery@^3.18.2` 从传递依赖提升为显式 devDependency | 三条不可合并的理由：① **注入点是值不是旗标**——blocked-arg 过滤器管得住「多出来的 flag」，管不住「值本身长成 flag / TOML 引号」（codex 的 `-c key="${effort}"` 是明文内联，一个 `"` 就换键）；一条共享正则同时覆盖六家比各自为政的私有判据更难漂移，且错误文案都落到同一形状（MI-20 的「报错必须指名值」原则推广）。② kind 是唯一真话——ACP v1 **没有**会话级 kind，旧代码按 id 信任就是把引擎的一句话当成权限证据；收紧后 id 偏好仍在（one-shot kind 下 allow_session 仍优先），但永远不再升级授权面。③ send 的旧描述是它自己埋的雷：`resumed: snapshot.sessionId === args.sessionId` 在 kernel 恒新 mint id 的结构下**永假**，而描述却把「永假」讲成了「有时会开新会话」——字段和文案一起换成结构里真实存在的两个事实。**跳过三案的裁决**（L3 prompt 走旗标值 / L7 unref / L8 onOverflow 排序）：各行文件头已写入分析，不构成行动。 | ✅ 已实现（**先红后绿自证**：15 条新/改断言先行全红 15/15，实现后目标 10 文件 297/297 绿；`tsc` src+tests 0 错误；门禁 `gates.sh` vitest/tsc×2/build×2 五道 OK，verify 外部 skill 不在沙箱 SKIP；全量 vitest **1064 passed / 7 skipped**（7 条为平台条件跳过）。fixture `fake-acp-cli.mjs` 的 permission 场景扩为 4 问：会话 id+one-shot kind 仍优先（正控）、会话外观+permanent kind 落选（M2 负控）、permanent+reject 取 reject、permanent-only 协议错误。registry 新增真进程负控：sentinel 孙进程在 timeout SIGKILL 后 `pgrep` 全灭——修复前确实残留。`package.json`+`pnpm-lock.yaml` 进 git，frozen-lockfile 安装自洽；lockfile 里 `@pnpm/exe` 的包管理器依赖段是 pnpm 12 自愈项，手删会被 `pnpm install` 写回，**保留不动**。skip 理由：L3 最坏只是 parse 失败（zcode/openclaw 的 prompt 是 flag 的值不是裸 positional）、L7 定时器改动面与收益不匹配、L8 仅 256MiB 以上触发且现行 failed 报告是诚实失败） |
| D44 | **ACP 引擎常驻（进程保活 + 会话复用）**：新增 `src/drivers/acp-resident.ts` 进程池（key = `agentId::cwd`），`acpKeepAliveMs`（默认 1h，`DSH_AGENTS_BRIDGE_ACP_KEEPALIVE_MS` 可覆盖，`<=0` 关闭）开启后，跑完**不杀引擎进程**，`client` 连同 ACP 会话一起入池；下次 `agents_run` 直接 adopt 同一进程，跳过 spawn → initialize → 命令表加载，只付 `session/new` + 拨盘 + `session/prompt`；`agents_send` 续接同一后端会话时连 `session/new` 和模型/effort 拨盘都跳过。取消/超时/失败路径会 `evict` 出池并回收进程组；插件卸载时 `dispose()` 全量回收。 | 用户明确要求「只有首次初始化可接受」——实测 qoderclicn 每次冷启动 ~24-29s，瓶颈是 spawn + ACP 握手 + 270 命令表加载，而非模型本身。ACP 引擎本就是持久服务（答完一轮继续等 stdin），旧实现每次 `client.shutdown()` 主动杀进程才导致每次付全量冷启动；常驻后第二次调用只剩一次 `session/prompt` 的往返。并发安全：条目 `inUse` 单占用，并发 run 直接新起进程而非排队；`cwd` 进 key 是因为 ACP 会话的 cwd 在 `session/new` 固定，跨目录复用会污染会话。 | ✅ 已实现（`acp.ts` 增加 adopt/release/evict 三态与 keep-alive settle 分支，`drivers/index.ts` 透传池，`src/index.ts` 接入插件生命周期并默认开启；`tests/drivers/acp-resident.test.ts` **5 条新测试全绿**：第二次 run 复用同一 pid、resume 复用同一 pid+同一后端会话、空闲回收、dispose 全回收、禁用回退一进程一会话；`tsc` src+tests 0 错误；ACP 套件 **66/66**；verify 11/11；全量 vitest **1069 passed / 6 failed / 1 skipped**，6 例均为 `tests/kernel/spawn.test.ts` 的本机 locale 环境基线（`processStartTimeMs` 在 de_DE/zh_CN 下返回 undefined），与本次改动无关） |
| D45 | **Qoder 接入全面对齐 multica（参考实现）**：① `qoderclicn` CLI 身份启动参数从 `--acp` 改为 `--yolo --acp`，与桌面 `qoder-cn` 行、与 multica `qodercli --yolo --acp` 完全一致；② ACP driver 的模型拨盘从 `session/set_config_option {configId:"model"}` 换成 multica 同款 `session/set_model {sessionId, modelId}`；③ 因 `set_model` 响应是 `{}`，`AcpClient` 新增 `latestConfigOptions` 捕获 `config_option_update` 通知（真机实测该通知**先于**响应到达），effort 仍对着**设完模型后**的档位表校验。 | 用户要求「全部跟 multica 的实现对齐」。multica（Go 后端 `server/pkg/agent/qoder.go`）是同一 ACP 线的生产参考实现：`--yolo` 是 headless 权限旁路开关（真机实测带它 `currentModeId=yolo`，不带落在 `default` 且工具会弹批准）；`session/set_model` 是它的模型杠杆。真机探测（qoderclicn 1.1.56）：`set_model {modelId:"qmodel"}` 与 `{modelId:"qfmodel"}` 均 **OK `{}`**，假 id `NOT_A_REAL_MODEL` 报 **-32602 "Invalid or unavailable model"** —— 与 `set_config_option` 一样被校验，且是标准 ACP 方法。`session/new` 的 `model` 参数保留（ACP 标准字段，multica 的 hermes 后端也发）。**对齐前先证伪**：用最小多帧脚本照搬 multica 的 `--yolo --acp` + `initialize(protocolVersion:1)` + `session/new` + `session/prompt`，结果同样在 `session/prompt` 得到上游 500 —— 即 Qoder ACP 后端当前对**所有**客户端实现都报 500，与 `--yolo`、模型、桥代码均无关。 | ✅ 已实现（`src/tracks/cli/catalog.ts` protocolArgs 加 `--yolo`，desktop 与 CLI 两行现在逐字相同；`src/drivers/acp.ts` 模型拨盘换 `session/set_model` + `latestConfigOptions` 通知捕获；fixture `fake-acp-cli.mjs` 的 `session/set_model` 改为有状态/校验/记 dial/通知先于响应；`qoderclicn-acp` / `qoder-cn-acp` / `hermes-acp` / `cli` / `acp` 测试断言与注释同步更新）。`tsc` src+tests 0 错误；受影响的 ACP + CLI 套件全绿。真机：`session/set_model` 被接受并校验（见理由栏），`session/prompt` 仍遇 Qoder 上游 500（与 multica 最小复刻结果逐字相同，属服务端故障）） |
| D46 | **Qoder CN CLI 增加 headless stream-json 传输 + 传输开关（默认 stream-json）**：ABI v9 新增 `ProtocolFamily += 'qoderclicn'`；`src/drivers/qoderclicn.ts` 是一个薄方言（复用 `runStreamJsonFamily`，不新写引擎）；CLI 轨新增身份 `qoderclicn-print`（`-p --output-format stream-json --input-format stream-json --permission-mode bypass_permissions`，prompt 走 stdin 的 claude 帧）。新增 `Config.qoderTransport`（`'stream-json' \| 'acp'`，env `DSH_AGENTS_BRIDGE_QODER_TRANSPORT`，默认 `'stream-json'`），实现方式是 descriptor override：把**未选中**的那一行标 `unsupported` 并写明理由。ACP 两行（`qoderclicn`、桌面 `qoder-cn`）与 headless 行**全部保留**，翻转开关即恢复。 | ① **为什么加**：ACP `session/prompt` 目前对**所有**客户端回上游 500（multica 序列逐字复现，见 D45），而 headless 正常 —— 用户要求「默认先使用 stream-json」。② **为什么是新方言而不是复用 codebuddy**：帧确实是 claude/CodeBuddy 那套（实测 `system/init`(session_id/model/tools/mcp_servers) → `assistant` → `result{subtype:success,result:"PONG"}`，每帧带 `session_id`），但旗标不同：`--verbose` 在这台 CLI 上 `error: unknown option`（两个现有 stream-json 方言都带它）、模式拼写是 `bypass_permissions`、effort 是 `--reasoning-effort`（`--effort` 同样报 unknown option）、工具表里没有 `AskUserQuestion` 可禁。③ **为什么用 `unsupported` 表达「未选中」而不是删行**：kernel 本就拒绝 unsupported 身份并给机器可读理由、probe 显示 `launch:'unsupported'`，模型能学到边界而不是桥静默隐藏；翻转设置零代码恢复，证据也不丢。未验证的三处（`--strict-mcp-config` 作用域、是否读自带项目上下文、权限客户端读哪个键）全部取保守侧并写明。 | ✅ 已实现（`src/drivers/qoderclicn.ts`（含 `effortFlag` 方言字段，默认 `--effort` 保证 claude/codebuddy 逐字不变）、`src/drivers/index.ts` 接线、`src/tracks/cli/catalog.ts` 新增 `qoderclicn-print`、`src/index.ts` 开关 + `resolveQoderTransport` + `qoderTransportOverrides`）。**穷尽性由编译器兜底**：新增 family 后 `tsc` 直接点名三处按 family 枚举的地方（driver/kernel 两张 idle 超时表、`DRIVER_FAMILIES`）与一处 schema 枚举；顺手把**两处手抄清单**改成派生（探测工具 schema 的 family enum ← `DRIVER_FAMILIES`、`argv.test.ts` 的「Known families」断言 ← 同一常量），消灭漂移源。测试：`tests/drivers/qoderclicn.test.ts` **6 条**（argv 拼写 + 负控：不含 `--verbose`/`bypassPermissions`/`--disallowedTools`、`--effort` 不出现、blocked map 按本 CLI 拼写）+ `tests/plugin-config.test.ts` **+5 条**开关用例（经 `agents_probe` 观察：默认 stream-json 时 ACP 行 reason 含 `qoderTransport=stream-json`、翻到 acp 时反之、桌面行两种模式下都不受影响、config > env > default 优先级、只标记未选中那一行）。`tsc` src+tests 0 错误。**真机验收**：`scripts/acceptance.ts qoderclicn-print "Reply with exactly: PONG"` → `probe available=true`、事件 `[status] running (model=Qwen3.8-Flash, permissionMode=bypassPermissions)` + `[thinking]` + `[text] PONG`、`status=completed exit=0`、`backendSessionId 9cc443f4-…`（来自帧回传的 session_id → resume 可用）） |
| D47 | **`qoderTransport` 进设置面板（补第一轮审查的 P4-1）**：settings 命名空间新增 `qoderTransport` 字段(新 `choice` 字段类型：闭集 `stream-json \| acp`，schema 用 `z.union` 拒绝界外值，`coerceField` 与 config/env 门同样做 trim+lowercase 归一)；卡片把 `choice` 渲染成 `<select>`（含「跟随部署配置」空档 = 清空该键）；**`apply()` 的开关决策改为 settings 用户层 > 插件配置 > env > 默认**——决策点从 config 字面后移到 `installSettings` 之后，读端口的解析值。| 面板只写 settings.yaml 的用户层；若 `apply()` 只读插件 config，面板保存会报 ok 却什么都不改——正是本模块要防的假开关（实现 P4-1 时发现的接线缺口）。用户层压过 config 与本命名空间其余字段的分层一致（`schema ← composition entry ← user`）。effect 仍是 `reload`：决策只在插件加载时做一次，卡片的「下次加载生效」保持诚实；三扇门优先级写入 README §4.1 与 D46 说明。| ✅ 已实现（`src/settings.ts` 字段+schema+choice 分支+`applyTo` 明确跳过本字段；`src/client/api.ts` wire 类型+选项过滤；`src/client/settings.ts` select 渲染；i18n 双语；`src/index.ts` 决策点后移并诚实标注 `source: settings`；README §4.1/D46；`docs/plan.md` 本行。测试：settings 套件 +3（闭集拒绝并指名全集合、不落 managerOptions、entry 携带+reset 回落）、plugin-config +2（用户层压过 config、空用户层回落 config，均经 `agents_probe` 观察）、client api +2（选项串过滤、无选项行保留）、components +2（select 选项精确、只读禁用）。`tsc` src+tests 0 错误；全量 vitest 见审查报告第二轮补记） |
## 任务拆分（3 个并行工作流）
