...
| 测试 | **745 个通过 + 1 skipped（45 passed \| 1 skipped 文件）** —— 基线 704/1；工作流 H 新增 6 个、D31 新增 3 个、`node-shim-note` 新增 6 个、`settings-surface` 新增 15 个（`tests/settings/settings.test.ts` 10 + `tests/host/settings-route.test.ts` 4 + `tests/client/plugin.test.ts` keyed 槽位 1）、**D35 新增 6 个**（同一文件 10 → 16）、**D36 新增 2 个**（`tests/client/components.test.ts` 20 → 22）、**D37 新增 3 个**（同一文件 22 → 25，已验证会真红），零删除、零跳过 |
| `tsc --noEmit` | 0 错误 |
