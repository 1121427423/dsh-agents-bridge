...
| `qoderclicn-acp-handshake.ndjson` | **CAPTURED** (frames verbatim; ONE frame OMITTED — see below) | **A DIFFERENT BINARY from the two rows above**, despite the same product: the standalone npm CLI `@qodercn-ai/qoderclicn` 1.1.56, i.e. `~/.nvm/…/bin/qoderclicn --acp` (NOT the app's bundled `qoder-worker-runtime.obf.mjs`, which is 1.1.53). Same host, 2026-09-19, with the operator's own CLI login in place. Frames: `initialize` result + `session/new` result (sessionId + 5 `modes` + 14 `models` + 3 `configOptions`) + one `agent_thought_chunk` + one `agent_message_chunk` + the `session/prompt` result (`stopReason: "end_turn"`, text `OK`). **The omitted frame** is again `available_commands_update`. This capture was taken with `--acp` and ONLY `--acp`, so `currentModeId` is `"default"` here. **D45 later aligned this row's launch with the desktop sibling and the reference implementation (`--yolo --acp`)**; because the `session/prompt` frame could not be re-taken while the Qoder ACP backend was answering upstream 500s, the re-capture lives in its own file (next row) and this full-turn capture is kept for its prompt result and message chunks. `agentInfo.version` is the field that tells the two Qoder captures apart — 1.1.56 here, 1.1.53 above — which is why the version is asserted in the test rather than assumed. |
| `qoderclicn-acp-yolo-session.ndjson` | **CAPTURED** (verbatim; trailing frames OMITTED) | the SAME standalone CLI, same host, 2026-09-19, re-captured under the **D45-aligned argv `--yolo --acp`**: real `initialize` result + `session/new` result, and that is all — `session/new` is the frame that carries the mode. `modes.currentModeId` is `"yolo"` here (versus `"default"` in the full-turn row above), which is the observable that proves `--yolo` really selects the headless bypass mode rather than being a no-op flag; `agentInfo.version` is 1.1.56 and the catalogue still advertises 14 models. **The omitted frames** are the two setup notifications and the `session/prompt` frame, which could not be taken at capture time: the Qoder ACP backend was answering `session/prompt` with an upstream `{code:500, message:"Sorry, something went wrong…"}` for every client (reproduced independently with a minimal multica-style script), so there was no successful prompt result to keep. Every frame present is byte-for-byte as it arrived. |
